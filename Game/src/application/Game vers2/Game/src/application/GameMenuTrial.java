...
//	private ArrayList<Group> random_block_list_2 = new ArrayList<>();
	...
	private double shield_start_time;
	
	/** The shield end time. */
	private double shield_end_time;
...
//	private class Timehandler_random_blocks_1 implements EventHandler<ActionEvent>{
//		@Override
//		public void handle(ActionEvent event){
//			check_block_type = false;
//			num_blocks_2 = random_block_list_2.size();
//			Random number = new Random();
//			int num_of_boxes = number.nextInt(6);
//			for(int i=0;i<num_of_boxes;i++){
//				int x_coord = number.nextInt(5);
////				block_list.add(rect.generate_blocks(i,snake_list.size()));
//				random_block_list_2.add(rect.generate_random_blocks(x_coord,snake_list.size()));
//			}
//			blocks2=Animate_blocks(random_block_list_2,num_blocks_2);
//			for(int u=0;u<blocks2.size();u++) {
//					blocks2.get(u).play();
//			}
//			for(int i=num_blocks_2;i<random_block_list_2.size();i++){
//				if(!inres) {
//					pane_game.getChildren().add(random_block_list_2.get(i));
//				}else {
//					pane_game_1.getChildren().add(random_block_list_2.get(i));
//				}
//			}
//		}
//	}
...
//				block_list.add(rect.generate_blocks(i,snake_list.size()));
	...
	private class Timehandler_Wall_1 implements EventHandler<ActionEvent>{
		@Override
		public void handle(ActionEvent event){
			Random r = new Random();
			int chance = r.nextInt(5);
//			num_shield = shield_list_2.size();
			if(chance == 3 || chance==2 || chance==4){
				num_walls = wall_list_2.size();
//				check_ = true;
//				double max = -10;
//				double min = 10000;
//				for(int i=num_balls;i<ball_list_2.size();i++){
...
//					}
//				}
				for_wall = new_wall.generateDestroyer();
				Group a=new Group(for_wall);
				wall_list_2.add(a);
				walls = Animate_wall(wall_list_2,num_walls);
				for(int u=0;u<walls.size();u++) {
					walls.get(u).play();
				}
//				System.out.println(num_shield);
				for(int i=num_walls;i<wall_list_2.size();i++){
				if(!inres) {
					pane_game.getChildren().add(wall_list_2.get(i));
				}else {
					pane_game_1.getChildren().add(wall_list_2.get(i));
				}
			}
		  }	
		}
	}	public ArrayList<TranslateTransition> Animate_wall(ArrayList<Group> list, int num){
...
//				System.out.println(num_shield);
	...
	private class Timehandler_Shield_1 implements EventHandler<ActionEvent>{
		...
		public void handle(ActionEvent event){
			...
			int chance = r.nextInt(5);
//			num_shield = shield_list_2.size();
			if(chance == 3 || chance==2 || chance==4 || chance==1 || chance==0){
				num_shield = shield_list_2.size();
				check_shield = true;
				double max = -10;
				double min = 10000;
...
//			a.play();
				...
				if(snake_list.get(0).getBoundsInParent().intersects(block_list_2.get(i).getBoundsInParent())){
					...
					int h=Integer.parseInt(gh.getText());
						if((!(inres))&&h>=snake_list.size()&&!shield) {
							...
							File f=new File("D:\\eclipse-workspace\\Game\\src\\application\\d.txt");
							FileWriter fw=null;
							try {
	...
	private class Timehandler_shield_3 implements EventHandler<ActionEvent>{
		...
		public void handle(ActionEvent event){
			...
			if((shield_end_time-shield_start_time) > 7){
				shield=false;
...
private class TimeHandler_balls_3 implements EventHandler<ActionEvent>{
	...
	public void start(Stage stage) throws Exception,InterruptedException{
		...
		try {
			inp1=new ObjectInputStream(new FileInputStream("D:\\eclipse-workspace\\Game\\src\\application\\p.txt"));
		...
		try {
			inp=new ObjectInputStream(new FileInputStream("D:\\eclipse-workspace\\Game\\src\\application\\d.txt"));
...
//		Pane pane_game = new Pane();
		...
		Image img3 = null;
		try{
			in3 = Files.newInputStream(Paths.get("D:\\eclipse-workspace\\Game\\src\\application\\black.jpg"));	
			img3 = new Image(in3);
		}finally{	
			in3.close();
		}
...
//			t_balls_2.setRate(0.5);
		...
		Image img4 = null;
		try{
			in4 = Files.newInputStream(Paths.get("D:\\eclipse-workspace\\Game\\src\\application\\black.jpg"));	
			img4 = new Image(in3);
		}finally{	
			in4.close();
		}
	...
	public static void main(String[] args){
		Media sound=new Media(new File("D:\\eclipse-workspace\\Game\\src\\application\\music.mp3").toURI().toString());
